(function () {
  ...
  const L4_DETAILS = {
    v3: {
      attention_q_compress: ["Linear projection", "Low-rank state"],
      ...
      attention_rope_compose: ["Apply Q RoPE", "Assemble query", "Assemble key"],
      attention_score: ["QK matmul", "Scale"],
    ...
    v3_2: {
      attention_q_compress: ["Read token_x", "Dq projection"],
      ...
      attention_kv_compress: ["Read token_x", "DKV projection"],
      attention_kv_split1: ["Split KV latent", "Split KR preimage"],
      attention_kv_norm: ["RMSNorm Ckv"],
      attention_kv_fp8_quant: ["Dynamic quant", "Write kv scale"],
      attention_kv_cache_update: ["Write kv_cache"],
      attention_kr_rope: ["KR projection", "RoPE", "Write kr_cache"],
      attention_idx_topk: ["QK matmul", "ReLU", "Weight multiply", "Group sum", "Top-k select"],
      attention_rope_compose: ["Gather selected KV", "Assemble Q", "Assemble K"],
      attention_sparse_attn: ["QK matmul", "Online softmax", "Weighted value matmul", "Normalize output"],
  ...
  function buildIndexerPrologL4(parentId, centerX, originY, width) {
    ...
    const mergeY = Math.max(queryPath.bottom, weightPath.bottom, keyPath.bottom) + L4_GAP;
    const exitPoint = point(centerX, mergeY + 6);

    [queryPath.lastNode, weightPath.lastNode, keyPath.lastNode].forEach((lastNode) => {
      if (!lastNode) return;
      const bottomAnchor = anchor(lastNode, "bottom");
      edges.push(
        edgeSpec("operator-detail", [
          bottomAnchor,
          point(bottomAnchor.x, mergeY),
          point(centerX, mergeY),
          exitPoint,
        ])
      );
    });
  ...
  function buildAttentionClusterV32(centerX, originY) {
    ...
    const krBranchX = anchor(krRope, "left").x - 24;
    ...
    const edges = [
      ...
      edgeSpec("detail", [
        anchor(krRope, "left"),
        point(krBranchX, anchor(krRope, "left").y),
        point(krBranchX, idxFanY),
        point(anchor(idxProlog, "top").x, idxFanY),
        anchor(idxProlog, "top"),
      ]),
